...
$z \in \{[0.20, 0.33),\allowbreak\,
[0.37, 0.50),\allowbreak\, [0.50, 0.65)\}$, which are the
standard DES~Y1 redshift edges with the upper edge of $z_0$ pulled in from $0.35 \to 0.33$ and the
...
\modname{analytical\_C1} $\to$ \modname{boost\_factor} $\to$ 
\modname{empirical\_mass\_match} $\to$
\modname{jackknife\_covariance} $\to$ \modname{save\_npz}}}
\end{center}
...
side-by-side. After implementing the boost factor and calculating the covariance matrices, the final stage persists everything to
...
\paragraph{Cosmology.} \texttt{FlatLambdaCDM(H0=70, Om0=0.286, Ob0=0.046,
...
We also compute the covariance matrix for the number counts by only considering the Poisson noise of the counts in each bin, which is a diagonal matrix with
\begin{equation}
  \mathrm{Cov}[N(\lob, \zob)]_{ii} = \sqrt{N(\lob, \zob)_i}.
\end{equation}.
...
\midrule
\multicolumn{3}{l}{\textit{Lensing geometry, C19/analytical route only (\S\ref{sec:units})}}    \\
\blk{scinv\_bin\_C19}         & $(N_\lambda, N_z)$     & $\Sigma_{\rm crit}^{-1}(\langle\zob\rangle_{\rm bin})$, physical [$\mathrm{pc}^2/M_\odot$] \\
\blk{z\_rep\_C19}             & $(N_\lambda, N_z)$     & bin-mean $\langle\zob\rangle$ used to evaluate \blk{scinv\_bin\_C19} \\
\blk{h\_buzzard}              & scalar                 & Buzzard's fixed $h = 0.70$ (DeRose+2019) \\
\blk{scinv\_bin\_C19\_littleh} & $(N_\lambda, N_z)$    & $\Sigma_{\rm crit}^{-1}$, little-h [$\mathrm{pc}^2/(h\,M_\odot)$] $= \blk{scinv\_bin\_C19}/\blk{h\_buzzard}$ \\
\blk{radii\_cMpc\_h\_C19}     & $(N_\lambda, N_z, N_R)$ & comoving $\hMpc$ coordinate of \blk{radii\_phys\_mpc}, per bin \\
\midrule
...
\subsection{Units and the $h$-convention}
\label{sec:units}

Every \blk{gamma\_t\_*} array is dimensionless
($\gamma_t = \Delta\Sigma \times \Sigma_{\rm crit}^{-1}$) and computed
self-consistently in physical units throughout, so it never needs an
$h$-conversion and is identical regardless of which \blk{scinv\_bin\_C19*}
convention below it is paired with:
\begin{itemize}
  \setlength\itemsep{2pt}
  \item \textbf{Physical} (\blk{radii\_phys\_mpc}, \blk{scinv\_bin\_C19}):
    proper Mpc and $\mathrm{pc}^2/M_\odot$, at Buzzard's fixed cosmology
    $h = 0.70$. This is an absolute-unit quantity, not a
    little-$h$-parametrized one --- Buzzard's cosmology is not a free
    parameter of the mock.
  \item \textbf{Little-$h$} (\blk{radii\_cMpc\_h\_C19},
    \blk{scinv\_bin\_C19\_littleh}): the standard weak-lensing convention,
    comoving $\hMpc$ and $\mathrm{pc}^2/(h\,M_\odot)$.
    \blk{gamma\_t\_mock\_obs\_C1} $/$ \blk{scinv\_bin\_C19\_littleh}
    recovers $\Delta\Sigma$ directly in $h\,M_\odot/\mathrm{pc}^2$.
\end{itemize}
Both \blk{*\_C19} fields are valid \emph{only} for the analytical/Matteo
route (\blk{gamma\_t\_*\_C1}/\blk{gamma\_t\_stack\_C19}) --- the
empirical/RM route (\blk{gamma\_t\_*\_RM}) uses its own internal bin-mean
redshift from the redMaPPer catalog (\S\ref{sec:emp}) that is not
currently exposed in the archive. \pyfile{output/MockDataVector\_targetR.npz}
(a separate, secondary artifact) is not covered by either convention above
and remains in its previous, unconverted physical-Mpc convention.
...
\begin{itemize}
  \setlength\itemsep{1pt}
  \item The Buzzard mock catalogues are described in DeRose et al.\ 2019, ApJS 242, 8.
  \item The parameters for the scaling relations in Eq.~\ref{eq:lsat} and projection in Eq.~\ref{eq:c19-a8} are taken from \href{https://github.com/MCostanzi/SelectionBias}{https://github.com/MCostanzi/SelectionBias}.
