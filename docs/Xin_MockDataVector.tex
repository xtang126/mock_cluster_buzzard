...
\modname{Xin\_MockDataVector.ipynb}: a Buzzard-halo DES~Y3-like cluster
...
$z \in \{0.20, 0.35, 0.50, 0.65\}$). The data vector is constructed along
\emph{two parallel routes} that share the same C19 forward model upstream
...
number-count vector $N(\lob, \zob)$. Boost-factor contamination is not
included (deferred work). All outputs are saved to
\pyfile{output/Xin\_MockDataVector.npz}; the headline plots embedded in
...
\pyfile{output/Xin\_MockDataVector.npz}.
...
bins are the DES~Y1 cluster edges (Costanzi+2019, McClintock+2019):
\begin{equation*}
  \texttt{LBDBINS}    = \{20,\, 30,\, 45,\, 60,\, 500\},\qquad
  \texttt{ZMIN\_LIST} = \{0.20,\, 0.35,\, 0.50\},\qquad
  \texttt{ZMAX\_LIST} = \{0.35,\, 0.50,\, 0.65\}.
\end{equation*}
The $500$ caps the open $[60, \infty)$ richness range. There are
$N_\lambda \times N_z = 4 \times 3 = 12$ bins.
...
(\texttt{l\_sat}, \texttt{l\_tr}, \texttt{sample\_lambda\_true}). The mean
satellite richness follows C26 Eq.~15,
\begin{equation}
\label{eq:lsat}
...
\end{equation}
clipped to zero below $M_{\rm min}$. The mean intrinsic richness is
$\langle\ltr|M,z\rangle = 1 + \langle\lambda_{\rm sat}|M,z\rangle$ for
$M \ge M_{\rm min}$ and zero below (C26 Sec.~III).
...
The per-halo $\ltr$ is drawn from a compound Poisson with multiplicative-
lognormal broadening of the Poisson mean (C26 Sec.~III):
\begin{equation*}
  m_{\rm broad} = m_{\rm sat}\,\exp\!\left[\sigma_{\rm intr}\mathcal N(0, 1)
  - \tfrac{1}{2}\sigma_{\rm intr}^2\right],\qquad
  N_{\rm sat} \sim \mathrm{Poisson}(m_{\rm broad}),\qquad
...
\end{equation}
The four parameters $\{\mu, \sigma, \tau, f_{\rm prj}\}$ depend linearly on
$\ltr$ with $z$-dependent coefficients (C19 Appendix~A, Eq.~A8); the 15
rows of \pyfile{data/prj\_params\_DESY3\_lss\_lin\_dep\_getdist\_v1.txt}
...
histograms) overlaid with the closed-form C19 Eq.~6 PDF (red dashed) at
three reference redshifts and three intrinsic richnesses. The two agree
within Poisson noise across the bulk and the tail.}
\label{fig:mc-vs-pdf}
\end{figure}

\paragraph{Sanity check 2 --- Gaussian core + EMG tail decomposition.}
Same Eq.~6 kernel, split into the two physical components. At large $\ltr$
...
\caption{Decomposition of the C19 Eq.~6 kernel into its
...
The number-counts vector is a histogram of the C19 forward-model $\lob$
on the observed-frame $\zob$, on the DES~Y1 grid:
\begin{equation*}
  N_{ij}^{\rm fwd}
  = \sum_{h} \mathbb 1\!\left[\lob_h \in [\lambda_i, \lambda_{i+1})\right]
    \mathbb 1\!\left[\zob_h \in [z_j, z_{j+1})\right].
\end{equation*}
The two binning quantities ($\lob, \zob$) come from the forward model
rather than from the catalog redMaPPer columns: the abundance side of the
data vector lives in the C26 model world, not in the cluster-finder world.
This is the correct entry for a C26 likelihood that integrates Eq.~15
...
\paragraph{Binning quantity.} The C19 forward-model
$(\lob, \zob)$. The C19 sampler is kernel-driven and depends only on
$(M, z)$: it carries no per-halo line-of-sight (LSS) information.

\paragraph{Selection bias.} The C26 Appendix~C fitting function
(5 constants, no integration). Matteo (priv.\ comm.) reports the same form
fits the bias on $\Delta\Sigma$:
\begin{equation}
\label{eq:c1-form}
...
\paragraph{Why this is useful in an MCMC.} The full C26 forward model
($\Sigma_{1h}$ via Eq.~12 $+$ $\Sigma_{\rm prj}$ via Eq.~13, then
Wright--Brainerd to $\Delta\Sigma$) involves nested line-of-sight integrals
through the optical-bias kernel $b_{\lob}$ (C26 Eq.~8) that are
prohibitive at every likelihood evaluation. Equation~\eqref{eq:c1-form}
collapses that calculation into five constants per richness/redshift bin.

\subsection{Selection-bias ratio $\Bcal_{\rm sel,C1}^{\Delta\Sigma}(R)$}
\label{sec:c1-ratio}
...
\subsection{Tangential shear with vs without the C1 correction}
\label{sec:c1-shear}
...
$\zob \in [0.35, 0.50)$ for each richness bin. Blue circles: $\gamma_t$
...
slice $\zrm \in [0.35, 0.50)$. Blue circles: $\gamma_t$ from the
...
\pyfile{output/Xin\_MockDataVector.npz}. With $N_\lambda = 4$, $N_z = 3$,
$N_R = 15$:
...
\midrule
\multicolumn{3}{l}{\textit{Number counts (forward-model side)}}                                 \\
\blk{NC}                  & $(N_\lambda, N_z)$     & $N(\lob, \zob)$ \\
\midrule
...
\begin{itemize}
  ...
  \item \textbf{Photo-$z$ kernel.} The Gaussian
    ...
    high-$z$ outlier rate.
  \item \textbf{Boost-factor contamination} not modelled. Cluster-member
    contamination of the source sample (Y3 boost factors) is the other
    major Y3 lensing systematic; it stacks on top of the
    optical-selection bias treated here.
...
\begin{itemize}
  ...
  \item \pyfile{Xin\_MockDataVector.ipynb} --- the production notebook.
  ...
  \item \pyfile{src/fileLoc.py} --- machine-resolved path table for the
    ...
    table.
  \item \pyfile{output/Xin\_MockDataVector.npz} --- saved data vector
    (Tab.~\ref{tab:archive}).
  \item \pyfile{output/figs/} --- headline plots embedded in this PDF.
  \item \pyfile{docs/Xin\_MockDataVector.tex} --- this document.
\end{itemize}
